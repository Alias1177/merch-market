...
4. **Проверьте состояние миграций (необязательно):**
   ```bash
   make migrate-status
   ```

5. **Остановите и откатите миграции (опционально):**
   ```bash
   make migrate-down
   ```
...
**Обратите внимание,тесты нужно запускать все вместе,пока дата база пустая,иначе могут возникнуть конфликты существования пользователя**

**Не забудьте очистить базу после тестирования!!!**
...
---
### Будущие планы на проект
- Оптимизирование тестирования(добавить отдельную дб для тестов)
- внедрение Kafka и Redis
